...
RV_array = pd.DataFrame(index=Index, columns=['Time', 'Phase', 'RV', 'RV_error', 'Bisector', 'Telescope'])

RV_array['Time'] = raw_data[:,0]
RV_array['RV'] = raw_data[:,1]
RV_array['RV_error'] = raw_data[:,2]
RV_array['Bisector'] = raw_data[:,6]
RV_array['Telescope'] = raw_data[:,8]
...
for i in RV_array.index:
    ...
    #print(RV_array.loc[i,'Time'] - Mid_transit)
    
    #print("phase:", RV_array.loc[i,'Phase'])
#endfor
...
RV_array_binned_telescopes = pd.DataFrame(index=Index2, columns=['Time', 'Phase', 'RV', 'RV_error', 'Bisector'])
...
for i in range(unique_time):
    ...
    RV_array_binned_telescopes.loc[i,'Bisector'] = np.average(RV_array.Bisector.drop(RV_array[RV_array.Time != unique_time_array[i]].index))
#endfor

Index3 = np.arange(unique_days)
RV_array_binned_days = pd.DataFrame(index=Index3, columns=['Time', 'Phase', 'RV', 'RV_error', 'Bisector'])
...
for i in range(unique_days):
    ...
    RV_array_binned_days.loc[i,'Bisector'] = np.average(RV_array.Bisector.drop(RV_array[RV_array.Time.round() != unique_days_array[i]].index))
#endfor
...
RV_array_binned_days_telescope1 = pd.DataFrame(index=Index4, columns=['Time', 'Phase', 'RV', 'RV_error', 'Bisector', 'Telescope'])
#Create 1 day bins of RV data together from the different telescopes
for i in range(unique_days_telescope1):
    ...
    RV_array_binned_days_telescope1.loc[i,'RV_error'] = np.sqrt(np.sum(RV_array_telescope1.RV_error.drop(RV_array_telescope1[RV_array_telescope1.Time.round() \
                                                                ...
                                                                != unique_days_array_telescope1[i]].index))
    RV_array_binned_days_telescope1.loc[i,'Bisector'] = np.average(RV_array_telescope1.Bisector.drop(RV_array_telescope1[RV_array_telescope1.Time.round() \
                                                                   != unique_days_array_telescope1[i]].index))
...
RV_array_binned_days_telescope2 = pd.DataFrame(index=Index5, columns=['Time', 'Phase', 'RV', 'RV_error', 'Bisector', 'Telescope'])
#Create 1 day bins of RV data together from the different telescopes
for i in range(unique_days_telescope2):
    ...
    RV_array_binned_days_telescope2.loc[i,'RV_error'] = np.sqrt(np.sum(RV_array_telescope2.RV_error.drop(RV_array_telescope2[RV_array_telescope2.Time.round() \
                                                                ...
                                                                != unique_days_array_telescope2[i]].index))
    RV_array_binned_days_telescope2.loc[i,'Bisector'] = np.average(RV_array_telescope2.Bisector.drop(RV_array_telescope2[RV_array_telescope2.Time.round() \
                                                                   != unique_days_array_telescope2[i]].index))
...
RV_array_binned_days_telescope3 = pd.DataFrame(index=Index6, columns=['Time', 'Phase', 'RV', 'RV_error', 'Bisector', 'Telescope'])
#Create 1 day bins of RV data together from the different telescopes
for i in range(unique_days_telescope3):
    ...
    RV_array_binned_days_telescope3.loc[i,'RV_error'] = np.sqrt(np.sum(RV_array_telescope3.RV_error.drop(RV_array_telescope3[RV_array_telescope3.Time.round() \
                                                                ...
                                                                != unique_days_array_telescope3[i]].index))
    RV_array_binned_days_telescope3.loc[i,'Bisector'] = np.average(RV_array_telescope3.Bisector.drop(RV_array_telescope3[RV_array_telescope3.Time.round() \
                                                                   != unique_days_array_telescope3[i]].index))
...
RV_array_binned_days_telescope4 = pd.DataFrame(index=Index7, columns=['Time', 'Phase', 'RV', 'RV_error', 'Bisector', 'Telescope'])
#Create 1 day bins of RV data together from the different telescopes
for i in range(unique_days_telescope4):
    ...
    RV_array_binned_days_telescope4.loc[i,'RV_error'] = np.sqrt(np.sum(RV_array_telescope4.RV_error.drop(RV_array_telescope4[RV_array_telescope4.Time.round() \
                                                                ...
                                                                != unique_days_array_telescope4[i]].index))
    RV_array_binned_days_telescope4.loc[i,'Bisector'] = np.average(RV_array_telescope4.Bisector.drop(RV_array_telescope4[RV_array_telescope4.Time.round() \
                                                                   != unique_days_array_telescope4[i]].index))
...
RV_array_binned_days_telescope5 = pd.DataFrame(index=Index8, columns=['Time', 'Phase', 'RV', 'RV_error', 'Bisector', 'Telescope'])
#Create 1 day bins of RV data together from the different telescopes
for i in range(unique_days_telescope5):
    ...
    RV_array_binned_days_telescope5.loc[i,'RV_error'] = np.sqrt(np.sum(RV_array_telescope5.RV_error.drop(RV_array_telescope5[RV_array_telescope5.Time.round() \
                                                                ...
                                                                != unique_days_array_telescope5[i]].index))
    RV_array_binned_days_telescope5.loc[i,'Bisector'] = np.average(RV_array_telescope5.Bisector.drop(RV_array_telescope5[RV_array_telescope5.Time.round() \
                                                                   != unique_days_array_telescope5[i]].index))
...
RV_array_binned_days_telescope6 = pd.DataFrame(index=Index9, columns=['Time', 'Phase', 'RV', 'RV_error', 'Bisector', 'Telescope'])
#Create 1 day bins of RV data together from the different telescopes
for i in range(unique_days_telescope6):
    ...
    RV_array_binned_days_telescope6.loc[i,'RV_error'] = np.sqrt(np.sum(RV_array_telescope6.RV_error.drop(RV_array_telescope6[RV_array_telescope6.Time.round() \
                                                                ...
                                                                != unique_days_array_telescope6[i]].index))
    RV_array_binned_days_telescope6.loc[i,'Bisector'] = np.average(RV_array_telescope6.Bisector.drop(RV_array_telescope6[RV_array_telescope6.Time.round() \
                                                                   != unique_days_array_telescope6[i]].index))
...
plt.rc('font', family='Times')
if len(RV_array_telescope1) > 0:
    plt.errorbar(RV_array_telescope1["RV"] - np.median(RV_array.RV), RV_array_telescope1["Bisector"], xerr=RV_array_telescope1["RV_error"], fmt='v',
                 ecolor='b', color='b', elinewidth=1, markersize=3, capsize=4, label='Telescope 1')
if len(RV_array_telescope2) > 0:
    plt.errorbar(RV_array_telescope2["RV"] - np.median(RV_array.RV), RV_array_telescope2["Bisector"], xerr=RV_array_telescope2["RV_error"], fmt='^',
                 ecolor='g', color='g', elinewidth=1, markersize=3, capsize=4, label='Telescope 2')
if len(RV_array_telescope3) > 0:
    plt.errorbar(RV_array_telescope3["RV"] - np.median(RV_array.RV), RV_array_telescope3["Bisector"], xerr=RV_array_telescope3["RV_error"], fmt='o',
                 ecolor='k', color='k', elinewidth=1, markersize=3, capsize=4, label='Telescope 3')
if len(RV_array_telescope4) > 0:
    plt.errorbar(RV_array_telescope4["RV"] - np.median(RV_array.RV), RV_array_telescope4["Bisector"], xerr=RV_array_telescope4["RV_error"], fmt='s',
                 ecolor='r', color='r', elinewidth=1, markersize=3, capsize=4, label='Telescope 4')
if len(RV_array_telescope5) > 0:
    plt.errorbar(RV_array_telescope5["RV"] - np.median(RV_array.RV), RV_array_telescope5["Bisector"], xerr=RV_array_telescope5["RV_error"], fmt='*',
                 ecolor='c', color='c', elinewidth=1, markersize=3, capsize=4, label='Telescope 5')
if len(RV_array_telescope6) > 0:
    plt.errorbar(RV_array_telescope6["RV"] - np.median(RV_array.RV), RV_array_telescope6["Bisector"], xerr=RV_array_telescope6["RV_error"], fmt='D',
                 ecolor='m', color='m', elinewidth=1, markersize=3, capsize=4, label='Telescope 6')
...
if len(RV_array_telescope1) > 0:
    ...
    if Allesfitter_format:
        allesfitter_out = directory + 'MINERVA_Tel1' + '.csv'
        RV_array_telescope1['RV'] = RV_array_telescope1['RV']/1000.0
        RV_array_telescope1['RV_error'] = RV_array_telescope1['RV_error']/1000.0
        RV_array_telescope1.to_csv(allesfitter_out, index=False, columns=['Time', 'RV', 'RV_error'], header=['# time', 'rv', 'rv_err'], na_rep='NaN')
...
if len(RV_array_telescope2) > 0:
    ...
    if Allesfitter_format:
        allesfitter_out = directory + 'MINERVA_Tel2' + '.csv'
        RV_array_telescope2['RV'] = RV_array_telescope2['RV']/1000.0
        RV_array_telescope2['RV_error'] = RV_array_telescope2['RV_error']/1000.0
        RV_array_telescope2.to_csv(allesfitter_out, index=False, columns=['Time', 'RV', 'RV_error'], header=['# time', 'rv', 'rv_err'], na_rep='NaN')
...
if len(RV_array_telescope3) > 0:
    ...
    if Allesfitter_format:
        allesfitter_out = directory + 'MINERVA_Tel3' + '.csv'
        RV_array_telescope3['RV'] = RV_array_telescope3['RV']/1000.0
        RV_array_telescope3['RV_error'] = RV_array_telescope3['RV_error']/1000.0
        RV_array_telescope3.to_csv(allesfitter_out, index=False, columns=['Time', 'RV', 'RV_error'], header=['# time', 'rv', 'rv_err'], na_rep='NaN')
...
if len(RV_array_telescope4) > 0:
    ...
    if Allesfitter_format:
        allesfitter_out = directory + 'MINERVA_Tel4' + '.csv'
        RV_array_telescope4['RV'] = RV_array_telescope4['RV']/1000.0
        RV_array_telescope4['RV_error'] = RV_array_telescope4['RV_error']/1000.0
        RV_array_telescope4.to_csv(allesfitter_out, index=False, columns=['Time', 'RV', 'RV_error'], header=['# time', 'rv', 'rv_err'], na_rep='NaN')
...
if len(RV_array_telescope5) > 0:
    ...
    if Allesfitter_format:
        allesfitter_out = directory + 'MINERVA_Tel5' + '.csv'
        RV_array_telescope5['RV'] = RV_array_telescope5['RV']/1000.0
        RV_array_telescope5['RV_error'] = RV_array_telescope5['RV_error']/1000.0
        RV_array_telescope5.to_csv(allesfitter_out, index=False, columns=['Time', 'RV', 'RV_error'], header=['# time', 'rv', 'rv_err'], na_rep='NaN')
...
if len(RV_array_telescope6) > 0:
    ...
    if Allesfitter_format:
        allesfitter_out = directory + 'MINERVA_Tel6' + '.csv'
        RV_array_telescope6['RV'] = RV_array_telescope6['RV']/1000.0
        RV_array_telescope6['RV_error'] = RV_array_telescope6['RV_error']/1000.0
        RV_array_telescope6.to_csv(allesfitter_out, index=False, columns=['Time', 'RV', 'RV_error'], header=['# time', 'rv', 'rv_err'], na_rep='NaN')
